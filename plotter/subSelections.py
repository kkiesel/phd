...
def subSelection(sampleName, dataset, totalDir, subDirs, name, binning, binningName):
    ...
    l = aux.Label(sim=dataset not in [data,dataHt], info=dataset.label)
...
def subSelections(sampleName, dataset):
    ...
    for name in names:
        for binningName, binning in aux.getBinningsFromName(name).iteritems():
            ...
            subSelection(sampleName+"_electrons", dataset, "tr", ["tr_gen0","tr_gen-11", "tr_gen11", "tr_gen-22", "tr_gen22"], name, binning, binningName)
...
if __name__ == "__main__":
    ...
    subSelection("data", dataHt, "tr_jControl", ["tr_jControl_noLep"], "met", aux.getBinningsFromName("met")["3"], "3")
